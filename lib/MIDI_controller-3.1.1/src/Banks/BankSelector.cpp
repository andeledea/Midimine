...
void BankSelector::refresh()
{
    ...
    case ENCODER_LEDS:
    {
        int32_t reading = enc->read();
        
        if (reading < prev_read) {
            newBankSetting = bankSetting < nb_settings - 1 ? bankSetting + 1 : 0;
        }
        if (reading > prev_read) {
            newBankSetting = bankSetting == 0 ? nb_settings - 1 : bankSetting - 1;
        }
        prev_read = reading;
    }
	break;
